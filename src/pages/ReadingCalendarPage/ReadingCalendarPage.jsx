...
function ReadingCalendarPage() {
  ...
  useEffect(() => {
    const fetchCalendar = async () => {
      ...
      try {
        ...
        if (!response.ok) {
          setMessage(data.message || "Не вдалося завантажити календар");
          return;
        }
  ...
  const getIntensityClass = (seconds = 0) => {
    if (seconds >= 3600) {
      ...
      <section className="reading-calendar">
        <div className="reading-calendar__header">
          ...
          <div>
            ...
            <p>
              {calendar?.booksCount ?? 0}{" "}
              {calendar?.booksCount === 1 ? "книга" : "книг"}
            </p>
          </div>
        ...
        {loading ? (
          ...
          <p className="reading-calendar__message">{message}</p>
        ) : (
          <div className="reading-calendar__grid">
            {days.map((day, index) => {
              ...
              const readingDay = readingByDay.get(day);

              const firstBook = readingDay?.books?.[0];

              const coverUrl = firstBook?.coverUrl
                ? firstBook.coverUrl.startsWith("/uploads")
                ...
                >
                  <div className="calendar-day__media">
                    {coverUrl && (
                      <img
                        src={coverUrl}
                        alt={firstBook?.title || "Обкладинка книги"}
                      />
                    )}

                    {readingDay?.books?.length > 1 && (
